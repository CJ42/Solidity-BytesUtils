...
library BytesUtils {
  /// @dev adapted from Nick Johnson (https://ethereum.stackexchange.com/a/4177/21704)
  function toBytes(uint256 _input) internal pure returns (bytes memory result) {
    result = new bytes(32);
    assembly {
  ...
  function getFirstHalfBytes(bytes32 _data) internal pure returns (bytes16) {
    return bytes16(_data);
  }

  function getLastHalfBytes(bytes32 _data) internal pure returns (bytes16) {
  ...
  // returns:   0xcafecafe
  function getFirst4Bytes(bytes32 _data) internal pure returns (bytes4) {
    return bytes4(_data);
  }

  function getLast4Bytes(bytes32 _data) internal pure returns (bytes4) {
  ...
  //
  function andingWithMask(bytes1 _value, bytes1 _mask)
    internal
    pure
    returns (bytes1)
  {
  ...
  // not working yet
  // function getFirstHalfBytesTest(bytes32 _value) internal pure returns(bytes16) {
  ...
  // not working yet
  function getLastHalfBytesTest(bytes32 _value)
    internal
    pure
    returns (bytes16)
  {
    uint256 lastN = uint256(_value) % (2**128);
  ...
  // not working
  function getFirstNBytes(bytes1 _x, uint8 _n) internal pure returns (bytes1) {
  ...
  function toBinaryString(uint256 _input)
    internal
    pure
    returns (string memory)
  {
    ...
    for (uint256 ii = 0; ii < 32; ii++) {
      output[32 - ii] = (_input % 2 == 1) ? bytes1("1") : bytes1("0");
      _input /= 2;
    }
  ...
  //         output[length - ii] = (_input % 2 == 1) ? bytes1("1") : bytes1("0");
  //         uint256(_input) /= 2;
  //     }
